...
function UserDropdown({
  ...
  userId: string | null;
  onUserIdChange: (userId: string | null) => void;
  users: { id: string; runescapeName: string; accountType: string; trackingStatus: string }[];
  loading: boolean;
  error: unknown;
}) {
  ...
  return (
    <div className="relative">
        ...
        <div className="flex items-center gap-2">
          {selectedUser ? (
            <>
              ...
              <span className="truncate">{selectedUser.runescapeName}</span>
            </>
          ) : (
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-background border border-border rounded-lg shadow-lg max-h-[60vh] sm:max-h-96 overflow-hidden">
              ...
              <div className="p-1">
                {filteredUsers.map((user) => (
                  ...
                  >
                    ...
                    <span className="flex-1 text-left truncate">{user.runescapeName}</span>
